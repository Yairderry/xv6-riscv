...
// what if two children exit(), 0 at the same time?
void
twochildren(char *s)
{
...
// locking order when giving away a child to init, so that exit(), 0
// deadlocks against init's wait()? also used to trigger a "panic:
// release" due to exit(), 0 releasing a different p->parent->lock than
// it acquired.
void
reparent2(char *s)
{
...
// run each test in its own process. run returns 1 if child's exit(), 0
// indicates success.
int
